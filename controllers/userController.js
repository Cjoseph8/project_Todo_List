   ...
   if (password.length < 6) {
    return res.status(400).json({ message: "Password must be at least 6 characters long" });
        ...
        const token = jwt.sign({
            ...
            process.env.jwtSecret, { expiresIn: "60 mins" })
...
exports.resendVerificationEmail = async (req, res) => {
    try {
        ...
        // create a token
        const token = await jwt.sign({ email }, process.env.jwtSecret, { expiresIn: "50m" });
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        const resetToken = await jwt.sign({ userId: user._id }, process.env.jwtSecret, { expiresIn: "30m" });

        // Send reset password email
        const mailOptions = {
            email: user.email,
            subject: "Password Reset",
            html: `Please click on the link to reset your password:<a>Reset Password</a> link expires in 30 minutes`,
        };
...
exports.login = async (req, res) => {
    try {
        ...
        const token = jwt.sign({
            ...
            process.env.jwtSecret, { expiresIn: "50 mins" })
...
exports.changePassword = async (req, res) => {
    try {
        ...
        const crossCheckPassword = await bcrypt.compare(existingPassword, user.password);
